...
class ValueGradFunction(object):
    ...
    def __init__(self, cost, grad_vars, extra_vars=None, dtype=None,
        ...
        grad = tt.grad(self._cost_joined, self._vars_joined)
        ## NOTE: grad takes the scalar y and the scalar or vector (list) of X's for the dy/pDx
        ## NOTE: it also takes a long time.
    ...
    def _build_joined(self, cost, args, vmap):
        ...
        args_joined = tt.vector('__args_joined') ## NOTE: <class 'theano.tensor.var.TensorVariable'>
        ...
        for vmap in vmap:
            ...
            ## NOTE: becomes a dict {'beta': beta, 'alpha': alpha}

        replace = {var: joined_slices[var.name] for var in args}
        #return args_joined, theano.clone(cost, replace=replace)

        ## NOTE: becomes a dict {'beta': beta, 'alpha': alpha}, replaces the variables passed in here
        ## QUESTION: Why? The args joined may be a vector of the variables now all in a single TensorVariable.
        ## Then clones the other variables.
